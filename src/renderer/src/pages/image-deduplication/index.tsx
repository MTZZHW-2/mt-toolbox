...
export default function ImageDeduplication() {
  const toolMeta = useToolMeta('image-deduplication');
  ...
  const handleStartDeduplication = async () => {
    ...
    try {
      // 注册进度监听
      const unsubscribe = window.api.onImageDeduplicationProgress((message) => {
        addLog(message);
      });

      const result = await window.api.imageDeduplication({
        sourceDir,
        similarityThreshold: parseInt(similarityThreshold) || 20,
        autoDelete,
      });
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="sourceDir">源图片目录 *</Label>
              <div className="flex gap-2">
                <Input id="sourceDir" value={sourceDir} readOnly placeholder="选择包含图片的目录" className="flex-1" />
